...
export function validateOpportunity(o: any, index = 0): string[] {
  ...
  if (!OPPORTUNITY_CATEGORIES.includes(o.category)) p.push(`${where}: bad category "${o.category}"`);
  ...
  if (!Array.isArray(o.eligibility)) p.push(`${where}: eligibility must be an array`);
    ...
    o.eligibility.forEach((r: any, i: number) => {
      if (typeof r?.profileKey !== 'string') p.push(`${where}: eligibility[${i}] missing profileKey`);
      if (!['equals', 'includes', 'gte', 'lte', 'truthy'].includes(r?.operator)) p.push(`${where}: eligibility[${i}] bad operator`);
...
export async function fetchRemoteCatalog(url = REMOTE_URL): Promise<Opportunity[] | null> {
  ...
  try {
    ...
    const items: unknown[] = Array.isArray(json) ? json : Array.isArray(json?.items) ? json.items : [];
